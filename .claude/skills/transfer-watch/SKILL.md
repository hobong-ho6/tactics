...
## 팀 루프 (2026-08-11 3팀 → 2026-08-20 **4팀** 확장)

이적 시장은 팀별 정기 이벤트다. 매 실행은 **AVL → CHE → LIV → ATM** 순으로 돈다:
- **AVL(주 분석)**: 아래 §0~§5 전체 파이프라인.
- **ATM(2026-08-20 편입)**: CHE·LIV와 동일 취급. ⚠️ **라리가는 마감일이 PL(09-01)과 다를 수 있다 —
  스페인 마감일을 먼저 확인하고 STALE·트리거 계산에 반영할 것.** ✅ `teams.sofascore_id=**2836**`
  (2026-08-20 확정 — 403 상태에서 검색 크롤 URL·본문으로 검증. ⚠️ 77879은 여자팀이다). fotmob_id 9906도 검증됨.
  ⚠️ 선수별 sofascore_id는 **08-21 오전에 스쿼드 23/23 확정**됐으나 **같은 날 오후 SofaScore가 다시 전 엔드포인트 403**이 됐다
  — 이후 추가한 8명(`382c03f`)은 sofascore_id 없이 **FotMob `/api/data/teams?id=9906&tab=squad`** 로 등재했다.
  ⭐ ATM 그리드 경로는 SofaScore가 아니라 **WhoScored `matchCentreData`(Opta)** 다 — R1 matchId 1993898 선례.
  ⭐ ATM은 **잭슨(첼시) 경쟁 클럽**으로 이미 원장에 등장한다 — CHE 축과 겹치므로 중복 판정을 피할 것.
...
1. SofaScore API에서 선수 검색 → 최근 6경기(45분+, 히트포인트 15+) 히트맵 수집.
   ...
   **API는 sofascore.com 페이지 컨텍스트에서만 접근 가능** (claude-in-chrome javascript_tool).
   ...
   - ✅✅ **[2026-08-19 해제 — obs#256] SofaScore 차단이 풀렸다. 위 경로를 그대로 쓴다.**
     3팀 프리시즌 전수 재수집으로 map25 보유 경기가 2 → 10으로 늘었다. ⚠️ 이월 결함 2건:
     **이중 기록 21쌍**(같은 출전이 SofaScore·FotMob 두 소스로 들어가 **행 수 기반 질의가 이중 계상**한다)
     과 AVL `2026-08-01 Indonesia All Stars` 중복 matches 행.
     ⚠️ **아래 「전면 차단」 블록은 2026-08-14~08-19에만 유효한 이력이다 — 현재 상태로 읽지 말 것.**
     (2026-08-20 14시 회차가 이 블록을 현재 상태로 오독해 **「403이 8일째」로 잘못 보고**했고,
     톰모리·토신을 실측 가능한데도 PENDING MEASUREMENT로 남겼다. 대체 경로 전환 회차도 불필요하다.)
   - ⛔ **[이력 · 2026-08-14~08-19] SofaScore 전면 차단 — 위 경로가 작동하지 않았다.**
...
3. 커널 적합도: `core.kernel.Kernel('FC26').best_fit_slot(map25, regime_id, pos)` —
   ...
   - **파싱 함정 (2026-07-30)**: MAPS 블록의 **마지막 항목(`st_false9`)은 뒤에 개행이 없어**
     ...
     > ⚠️⚠️ **[2026-08-24 전수 정정 — 구 표는 CHE 8칸·LIV 9칸이 틀렸다]**
     > 구 표는 3팀만 담았고 **LIV 열이 AVL 값 사본**이었으며 CHE도 포메이션이 섞여 있었다.
     > 위 표는 **커널과 동일한 조회**(`SELECT x, slot_type FROM slots WHERE regime_id=? AND pos=?` + `fetchone()`)로
     > 4팀 전수 재생성한 것이다. **AVL 열만 원래 전부 맞았다.**
     > 주요 오류: LIV `LB 13→16 · LCB 37→36 · RCB 63→68 · RB 87→84 · LDM 38→45 · RDM 62→56 · LM 14→26 · RM 85→77 · CAM 50→47` ·
     > CHE `LCB 31→25 · CCB 50→57 · RCB 66→75 · LM 14→15 · RM 86→79 · ST 50→42`, 그리고
     > **CHE에는 LDM·RDM이 아예 없다**(구 표의 `LDM 50 · RDM 61`은 존재하지 않는 슬롯이었다 — CHE는 `LCM 45 · RCM 55`다).
     >
     > ⭐ **`formation`이 실효값을 정한다.** `slots`에는 팀마다 여러 포메이션이 있고
     > (AVL: 4-2-3-1 Wide·4-4-2 / CHE: **3-4-2-1**·5-4-1·3-4-3 / LIV: 4-2-3-1 Wide / ATM: 4-1-4-1),
     > **커널은 `formation`을 조건에 넣지 않고 `fetchone()`으로 첫 행을 집는다.**
     > AVL은 두 포메이션의 공유 pos가 x를 동일하게 맞춰 놨지만,
     > ⛔ **CHE는 8개 pos(CCB·LCB·LCM·LM·RCB·RCM·RM·ST)에서 포메이션마다 x가 달라** 실효값이
     > **삽입 순서상 첫 행인 `3-4-2-1`** 로 결정된다. 위 CHE 열은 그 실효값이다.
     > ⚠️ 5-4-1·3-4-3 기준으로 계산하려면 `formation`을 명시해 직접 조회해야 한다 — 커널 호출로는 안 된다.
