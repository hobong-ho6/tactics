...
13. **⭐⭐⭐ 같은 문제를 두 번째로 확인하면 규율이 아니라 「구조」를 없앤다**(2026-09-23 사용자 지시
    「앞으로 동일한 문제를 확인하면 **바로 구조를 없애서 수정**해줘」).
    주석·런북·「조심하자」는 **읽는 사람이 있어야** 작동하고, 읽는 것은 세션이라 확률이 1이 아니다.
    ⇒ 위에서부터 시도한다: **① 구조 제거**(그 실수가 문법적으로 불가능하게) → **② 단일 정본**(규칙을
    한 모듈에 두고 나머지는 import) → **③ 게이트**(커밋을 막는다) → **④ 스크립트**(손 절차를 한 줄로)
    → ⑤ 문서(위 넷이 **모두 불가능할 때만** — 판단이 필요한 축).
    ⛔ ④⑤에서 멈추지 않는다 — 검사기를 만들어도 **부르지 않으면 소용없다**(실증).
    ⭐ 실증: 백틱 사고는 「조심하자」로 **다섯 번** 실패했고 파일을 옮기는 데는 **한 번**이면 됐다.
    판정 기준·사례표는 [docs/70-lessons.md](docs/70-lessons.md) 머리말.
