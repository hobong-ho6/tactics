...
> **2026-09-13 KST** · PC `AD03230205ui-iMac.local` · `main` · 마지막 커밋 **`d44296e`+** · **✅ origin 일치** ·
> ⚠️ 미추적 폴더 `reports/ingame/*`(커밋 여부 사용자 판단). ⚠️ ~~스케줄 태스크 목록이 비어 있다~~ → **오판 정정(09-13)**: 목록은 정상(활성 3 = unifi 월 10:03 · match-watch 월 10:06 · henderson 일 09:07, 비활성 소진 6건 — 브뤼헤 d1/d2/d3, CHE 리즈 d1/d2/d3). ⭐ **비활성 소진 태스크가 09-13 05:48 UTC에 미실행분 catch-up으로 일제히 재발화했다**(브뤼헤 d3·CHE 리즈 d3·henderson) — 소진분은 **삭제**해야 중복 회차가 안 뜬다.
> **09-13 세션 요약**: ⑴ **PL R4 3경기 전량 수집·complete**(AVL 1-2 포레스트 report 39 · CHE 2-2 헐 40 · LIV 0-0 풀럼 41) — 실측 48명·국면 그리드·PPDA/def_x·서사 D+1 병합·처방 27행 재산출, obs#632~646.
> ⑵ ⭐ **부상·결장 축을 라이브 시스템으로 편입**(사용자 지시) — `player_status` export + `player.html` 상태 카드 + **as_of 이후 출전 교차검증**(모순 5건 자동 경고). 신규 3건: 마첸 발목·파우 토레스 햄스트링·고메스 징계.
> ⑶ 브뤼헤전 D+3은 **09-12에 이미 완료**(obs#630·631).
> **09-12 세션 요약(후반)**: 데이터 부족 8명 SofaScore 517행·안필드전 §2 31명 적재(obs#620), 첼시-리즈 report 36 complete(교체 5명 처방 보강), NULL team_code 1,432행 배정(obs#618), G13 확장 2건. **안필드전 리포트 완료(0-3)**.
> **09-12 세션 요약(전반)**: ⑴ 스쿼드 결손 축 보강(FotMob 15명, 조나단 데이비드 `fotmob_id` 해결) ⑵ ⭐ **FC25·FC26 출시판 게임스탯 176행 시계열**
> (`scripts/collect_futgg_history.py`, fut.gg 정의 API curl) + `player.html` 「버전별 변화」 표 ⑶ 종합평가 19명 갱신 → **스쿼드 전원 09-10 이후 기준**
> ⑷ ⛔ 리스 제임스 FC27 행 오염(로렌 제임스 값) 정정(obs#616). ✅ 음바예 `team_code` 재점검 → **동일 유형 109행 정정**(음바예 23·HB 12·알랑 72·로저스 2, obs#617). ✅ 이어서 **NULL 1,432행 전량 배정**(obs#618 — 우리 팀 731·외부 클럽·대표팀 701, teams +19코드, G13 대표팀 분류 확장, 무뇨스 report 3 처방 누락 보강). **team_code NULL 0.**
...
1. ✅ **브뤼헤전 D+3 완료(09-12 세션 내 — 스케줄 태스크 목록이 비어 있었다, ⚠️ 정기 2건·일회성 3건도 목록에 없음: 재확인 필요)** obs#630·631(세이스 국면 차이·가르나초 첫 직접 질문·글라스너 스즈키 롱킥). 헐전 국면 그리드 백필·PPDA 정의 통일(obs#628). 재검증 7항은 **포레스트전(09-12) 회차**에서 판정 — 이전 항목:
1-old. **P1 · 브뤼헤전 D+2(09-11 23:00)·D+3(09-12 23:00) 회차 확인** — UTV 전술 에피소드(D+1 예고) 회수 · 재검증 7항(report 35 §7)은 **09-12 포레스트전(PL R4 홈)** 에서 판정: 잭슨 국면 반응 ①, 풀백 좌우 복귀 ②, 무공 4-4-2 4번째 표본 ③, def_x 2번째 실측 ⑦. 포레스트 **백3+스텝업 CB** 단서(obs#568) 프리뷰에.
