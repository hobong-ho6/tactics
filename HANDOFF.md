...
> **2026-09-08 11:49 KST** · PC `AD03230205ui-iMac.local` · `main` · 마지막 커밋 **`ad1296b`** · **✅ origin 일치 · 워킹트리 깨끗**.
> 09-08 CHE 축(사용자 지시 3건): ⑴ 「첼시 아스날전 수집」 → 본수집은 이미 `complete`(report 34)여서 **D+2 추적** 수행 —
> 유튜브 전술 4편·감독 회견 전문 3편 전사, **당일 판정 정정 5건**, obs#519~526, `player_duties` 10행 addendum(`387a079`).
> ⑵ 소진 일회성 스케줄 **11건 전량 삭제**(`3b1d92d`·`d44cb72`). ⑶ ⭐⭐ **pos_class 포메이션 인식 재작성 + 전량 재계산**(migration 026,
> obs#527~530) + **국면 어휘 툴팁**(`ad1296b`) — 아래 「09-08 ⑸」.
...
- DB: players **200** · player_matches **4,252** · team_match_stats **70** · match_reports **34**
  ...
  player_shirt_numbers **116** · understat_player_matches **5,634** · observations **530** ·
...
- ⭐⭐ **헐 관점 결손(3회차 연속 0건) 해소** — 요크셔포스트 Stuart Rayner 전담 기자 평점(403 → **브라우저 우회**).
  ⛔ **D+1 판정 철회**: 「촐라키스의 3연속 최고 퍼포먼스 탓」 → 전담 기자는 **6점 하위권**(「Villa so toothless」), 최고점은 CB 이건 8점
  ⇒ **「리그 페이지 MVP」는 선방 기여 지표가 아니다**(obs#500). 부산물: 헐 두 결정기가 **모두 교체 직후**(obs#501).
- ⭐⭐ **Coaches' Voice 「0건」이 뒤집혔다**(2회차 오판) — ⑴ 무공 「compact 4-4-2 → 와이드 미드 하강 시 일시적 5-3-2」
  ⑵ ⭐⭐ 아스날의 일관된 득점 경로 = **「빌라 풀백–CB 하프스페이스」** ⇒ 「캐시 뒤 이음새」가 **체제 구조 결손으로 승격**(obs#502).
- ⭐⭐ **themastermindsite**: 무공 4-4-2 생성 메커니즘(수비 시 10번이 ST 옆으로) + 「struggle to find that tertiary pass」
  ⇒ **「전진 패스 회피」는 체제 지속 형질**(obs#503). ⛔ 두 전술 소스 **모두 6-2-2를 말하지 않는다** — 3회차 연속 Squawka 단독, 기각 방향.
- **사용자 지시 영상**(UTV × The Villans `hzPzYHm8QsU`) — ⭐⭐ **잭슨 피패스 정량**(XGstat 경유, MEDIUM): 헐전 14개 중 **두 10번 합 4개(¼)**,
  최다 공급자는 **좌측 풀백**(obs#504). ⭐ 논점 ①을 「유지」→**「층위 분리 필요」**로 정정(obs#505) — 잭슨은 자주 하강했고 문제는 **배후 러너 부재**.
  ⇒ 재검증 지표를 **「최저 터치 y·하강 빈도 분포」**로 변경(평균 위치 금지).
- ⛔ 0건 유지: 6-2-2 · TFA/BTL/Spielverlagerung · 🇰🇷 한국어 · expressandstar(페이월). Hull Live는 **요크셔포스트가 대체 → 목록 제거**.
- **브뤼헤전 사전 프리뷰**(`reports/match-watch/2026-09-08-avl-brugge-preview.md`, `match_reports` 행 없음).
  ⚠️ **브뤼헤 감독은 하옌이 아니라 이반 레코** — TFA의 하옌 자료(3-5-2) 적용 금지. ⭐ 레코가 홈에서 「가두지 않고 공격하겠다」 명시
  ⇒ 헐전(딥블록·74%)과 **정반대 국면**이라 「측정 불가」였던 **무공 정착 블록이 처음 측정된다**.

### 2026-09-08 ⑴ ATM 축 — ⭐⭐ 아틀레틱전 D+3로 3일 창 종결 · 당일 판정 정정 3건 (`1274101`, obs#506~516)

- ⭐⭐ **D+1·D+2가 두 번 「0건」으로 닫은 전술 유튜브 해소** — 발행 지연이 아니라 **검색 방법** 문제(위 규칙). Proyecto Hadi·Atlético Stats 전사 2편(auto-caption).
- **판정 정정 3건**: ⑴ ⚠️⚠️ **obs#452 인과 철회** — 초반 하이프레스는 **아틀레틱 쪽**이고 ATM이 측면 전환으로 깼다, 2실점은 「전술적 배경 없음」(obs#506)
  ⑵ 「C.로메로=백3」 미채택(단일 출처) → **4-4-2 유지 + 요렌테 전진**, ⭐ 요렌테 한 경기 5개 포지션(obs#508) ⑶ 2실점 기점은 **G.시메오네 피탈**, 인플레이 14초(obs#512).
- **강화**: 최종 5-4-1 독립 확인(obs#507) · 우측 편중은 **유일하게 작동한 진행 루트**(obs#509) · 그리말도 귀책이 수비 커버→온더볼로 이동(obs#511) ·
  ⭐⭐ obs#459가 ⓒ**「진짜 9번 부재」**로 재정의돼 3가설 병존(obs#510) · 원정 공격 노선 = 「의도된 새 플랜」(obs#514, 팬 매체 해석이라 중간 신뢰).
- **미도달 확정**(재시도 금지): theobjective·atleticodemadrid.com(봇) · liverpool.com · El Correo(페이월) · EITB 바스크어(영상만) · 슬로바키아어·덴마크어.
- ⛔ 감독 발언 3항목 **「미확인 확정」** — 회견에 **질문 자체가 없었다**(obs#493). `player_duties` 7행(202~208) — ⭐ 그리말도는 경기 한정 `position` 라벨로
  `UNIQUE(season,player_id,position)`을 우회해 **덮어쓰기 없이** 해소(선례 id=197).

### 2026-09-07 — 필수 채널 규약 2건 · xG 정책 확정 · 헐전 유튜브 26편 (obs#466·#472~480·#499)

- **AVL 필수 채널 3곳 확정**: UTV·The Villans + **1874**(`@The1874Podcast`) → docs/30 표 · SKILL §2-1. 1874의 가치는 **Jacob Tanswell(The Athletic) 정기 출연**(1~2티어).
- **전사 26+7편** → 헐전 §8-8 · `player_duties` 16행. 수확: 스즈키 「third center half in build-up」 영어권 최초(obs#474) · UCL 명단 23인(obs#476) ·
  만잠비 「pocket filler」≠정적 포켓(obs#477) · 「맥긴 인버트」 실측 충돌(obs#473) · ⭐ **가르나초는 최우선 타깃이 아니었다**(1순위 = 우측 윙어 음바예) ·
  ⭐ **에메리는 왓킨스+잭슨 2톱을 원했다**(obs#446과 정합) · 고레츠카 「라인 브레이킹 패서 아님」 ⇒ **dm_dlp 정본과 긴장, 첫 출전 시 대조**.
- ⭐⭐ **xG 개정 정책 확정**(obs#466): **저장값이 정본, 개정 추종 안 함.** 혼합 3행은 두고 교차 집계에서 `xg_source`로 거른다(docs/30 ⑧). 화면에 원천 노출.
  ⚠️ 게이트는 혼합 **개수만** 고정한다 — 집계 질의의 필터 여부는 검사하지 못한다.
