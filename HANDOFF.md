...
- ⚠️ **스케줄 정본은 `mcp__scheduled-tasks__list_scheduled_tasks`다** — `CronList`는 다른 시스템이라 0건으로 보인다
  ...
  `match-watch-weekly`(월 10:06, 다음 09-07) · ✅ ~~`avl-arsenal-followup-d3`~~ **09-04 실행 완료·소진** ·
...
- ⭐⭐ **아르테타 공식 프레서 전문**(obs#436): 에제 투입 후 **near-1v1 3회** 자인 ⇒ 후반 로우블록 붕괴의
  **상대측 정량 확증**(우리 실측 빅찬스 2와 정합). 「미드블록 → 로우블록 국면 평균」 정정이 아스날 측에서 독립 확증.
- 🔴 정정(obs#437): 여름 유출은 5명이 아니라 **6명 — 뤼카 디뉴 누락**(→PSG).
- 🔴 **고레츠카 종결**: 20명 명단 부재 확정. 「미확정」이 아니라 **부재증명**으로 닫았다.
- 미확보 확정: TFA·BTL(미게시·4회 확인) · 클럽 전술 영상 · AVR 페이월 · CV 도해 이미지.
...
- **결과: 이 경기 전술 분석은 추가 5개 언어에서도 전부 0건** — 총 **9개 언어** 확인 후에도 확보분 3건
  (Coaches' Voice·Sky MNF·Bains)이 전부다. **미확보 영역은 「도달 불가」가 맞았다.**
- 🔴 **그래도 서사 축에서 판정 영향 2건**: ① **왓킨스가 브라이턴전 출전을 거부했다**(🇩🇪·🇫🇷 2중, obs#439) ⇒
  인과 사슬 완결(거부 → 9번 부재 → 부엔디아 false9 실패 → 하이턴오버 19회 → 롱볼 우회). **브라이턴전은
  전술적 이상치가 아니라 이적 파행의 함수다.** ⛔1차 인용 미확보. ② **스페인 예상 선발의 좌측 와이드는
  가르나초**였다(obs#440) ⇒ 헤밍스의 「지정 아웃렛」이 **지정 기능인지 대안 부재의 결과인지 분리 불가**.
  ⭐ 마첸 케이스와 같은 유형 — **이 경기 좌측(LB 마첸·LM 헤밍스)은 양쪽 다 「대안 부재」 의심 구간**.
- obs#437 강화(🇳🇱): 디뉴 행선지 PSG 확정 + **에메리가 마첸 자리를 보강하려 했고** 맨유 오퍼는 빌라가 거절.
- ⭐⭐ **규약 2건**: obs#441 **다국어 축은 「선발 11명 국적·경유 리그 체크리스트」로 강제**한다(화제성을 따라가면
  구조적으로 누락된다 — 재점검 실효 3건으로 입증). obs#442 **「0건」은 검색 실패가 아니라 구조적 사실** —
  🇸🇪 매체는 린델뢰프를 상시 커버하는데도 이 경기 기사가 없다. ⇒ **국적 언어권은 「경기 분석」이 아니라
  「이적 서사」 축에서 수확**이 나온다(스즈키 €35m·PSG 무산 비화, 잭슨 초기지급 £47.5m·알론소 스쿼드 분리 훈련).
...
2. **P1 · PL Round 4** — AVL 헐시티전(09-05)은 **자동 4건 예약됨**(위 「자동화」). CHE·LIV·ATM R4는 전용 태스크 없음 —
   ...
   무공 4-4-2 · 캐시 이음새 · 전후반 분리 PPDA 병기) **+ ⑺에 09-04 추가분 「가르나초가 선발하면 헤밍스의
   최종라인 기능이 그에게 이전되는지」**(obs#440 — 이전되면 「지정 기능」이 아니라 「자리에 딸린 기능」).
   ⭐ 좌측 두 자리(LB 마첸·LM 헤밍스)는 **양쪽 다 「대안 부재」 의심 구간**이라 함께 본다.
