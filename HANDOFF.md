...
- ✅ **CHE 풀럼전 D+1 심층(obs#340)** — ⭐⭐ **초판이 표면만 봤다.** Telegraph가 백3의 실제 메커니즘을 명시 —
  **리스 제임스가 미드필드에서 상습적으로 백3로 내려가 아체암퐁·라크루아 두 CB를 전진하게 풀어 준다**
  (원인은 포파나 출장정지, 포메이션은 직전 친선 3경기 모두 3-4-2-1). ⇒ **재검증 항목 #2가 「설계」로 해소.**
  ⭐ 실측과 독립 수렴 3건(산체스 3중 정합 · 로저스 8.65↔SI 8.7 + **2·3번째 골 모두 관여** · 통제 실패 판정).
  **xG 동일 스냅샷 재수집 실행** — `xg_v` 1.70→**2.24**, `xg_op` 1.96/1.28 채움, 검산 2중 통과
