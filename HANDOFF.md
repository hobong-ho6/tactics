...
1. ⭐ **CHE 풀럼전 D+2(08-26)·D+3(08-27)** — 알론소 공식 전체 회견, 클럽 전술 영상,
   ...
   ⏰⏰ **같은 회차에 xG 계열을 한 스냅샷으로 재수집한다**(사용자 결정 C, 2026-08-25 · obs#337).
   현재 `xg_v`=1.70(08-25 오전)인데 재수집하면 2.24(Δ+0.54)라 `xg_op`를 채우면 「오픈플레이 > 전체」가 된다
   ⇒ **오픈플레이 칸을 NULL로 비워 둔 상태**다. FotMob `matchDetails?matchId=5795372` 한 응답에서
   `xG`·`xG open play`·`Blocked shots`를 동시에 받아 4개 컬럼을 함께 채우고,
   **「1.70」 인용 4곳**(원문 14·85행 · `obs#323` claim · `report25.tactical_description`)도 같이 갱신한다.
   결론은 뒤집히지 않는다(2.24 vs 1.33이면 obs#323 논지가 더 강해진다).
   ⭐ **AVL 브라이턴전(`5795369`)도 같이 정리** — 0.31↔0.29(Δ−0.02)로 게이트는 통과하지만
   세트피스 xG가 0.02 틀린다. 규칙은 `match-watch/SKILL.md` §3에 신설했다.
