...
> **2026-09-08 10:56 KST** · PC `AD03230205ui-iMac.local` · `main` · 마지막 커밋 **`b7c1ab4`** · **✅ origin 일치 · 워킹트리 깨끗**.
...
- ✅ **[09-08 10:5x 정리 완료]** 삭제 대상 5건 중 **3건을 삭제**했다(`-avl-hull-followup-d1` · `-dplus-liv-ipswich-d3` · `-dplus-atm-athletic-d2`).
  삭제 전 **각 회차의 산출 커밋을 확인**했다(`7c358d1`·`db7190f`·`5e1ecde`) — 소실 없음. `SKILL.md` 원문은 디스크에 남는다(프롬프트 복구 가능).
  ⚠️ 나머지 2건(**헐 d2 · `-atm-athletic-d3`**)은 **이미 목록에서 사라져 있었다**(09-08 09:15 조회에는 있었다). 09-08 오전 사용자 수동 실행 회차와
  관련된 것으로 보이나 **소멸 경로는 미확인** — 산출 커밋(`ca42991`·`1274101`)은 정상이다.
  ⇒ ⭐ 위 「자기를 띄운 태스크는 그 세션에서 못 지운다」 규약은 **반례가 관측됐다. 재확인 필요.**
- **남은 소진 일회성 8건**(전부 `enabled:false`, 방치 무해): `transfer-feed` · `supercup-collect` · `match-watch-brighton-villa-2026-08-23` ·
  `unifi-p0-august-daily-recovery-0828-imac` · `match-watch-r3-followup-d1/d2/d3` · `match-watch-avl-hull-2026-09-05`.
