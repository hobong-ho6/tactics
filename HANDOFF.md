...
- DB: players **200** · player_matches **4,252** · team_match_stats **70** · match_reports **34**
  ...
  `team_match_stats.xg_source` **62/70** · `game_system_changes` **18** · **reproduction_limits 12** · **ingame_captures 29** · observations **537** ·
...
0-1. ⭐ **P1 · 인게임 A/B — 1차 완료·2차 설계** — 테스트 1(id19 `?ZWKd#Zw6mre`) 15장 처리 완료(`ingame_captures` 1~15, obs#536): 스크립트 15/15 검증 ·
   **히트맵은 위치 기반** · 와이드 Attack 뱅크 미형성 실증(맥긴 자기진영 5.9%·가르나초 27.3%). ⭐ **테스트 2 완료**(id20 `AZYMfMjy8otg`, obs#537, captures 16~29): 맥긴 자기진영 **5.9→38.9%**, 재현성 기준선 T1↔T2 .86~.91(맥긴만 .71).
   다음 후보: 가르나초 winger/Attack → wideplm/Attack(2경기 연속 정본 우위) · 잭슨 Support→Versatile. 매 경기 팀 통계 화면 + 상대 포메이션 + 조작 선수 필수(테스트 2 조작 선수·상대 포메이션 미답).
