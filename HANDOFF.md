...
- ✅ **transfer-watch 21시 회차** — 켈리먼 **CONFIRMED**(구단 공식) · 사르 **신규 HIGH**(Romano HWG) ·
  음바예 **강등**(09시가 걸어둔 트리거가 L'Équipe 1티어로 발화) · **ATM 잭슨 DEAD→MEDIUM-HIGH 복원** ·
  왓킨스 원장 정정(4차 £45m는 이미 제출·거절됐다 — 6회차 「미발화」 기록이 틀렸다).
  ⭐⭐ **FIFA art.10 수치 충돌 해소** — 「6칸 중 4칸」(총량)과 「스트라스부르 1칸」(동일구단 3명)은
  **다른 조항이고 둘 다 참**이다. U21 예외는 club-trained 누적 조건이라 전원 미충족.
  **국내 임대는 대상 밖** ⇒ 첼시가 헐을 선호하는 이유가 규정으로 설명된다.
  파생: **에수구와 무드리크가 스트라스부르 잔여 1칸을 상호 배타적으로 다툰다.**
  ⚠️ 원장 결함 발견: **`players` 에수구 중복(103/140)** — 아래 「미해결」 참조.
...
2. ⏰ **`overall_assessment` 채움 확대** — 현재 **4/25**(22·23·24·25만). ATM 말라가전(21)·AVL 슈퍼컵(1)이 다음 대상.
   프리시즌 친선은 **사용자 확인으로 미수집 확정**(일반 매체 총평 자체가 희박).
   ⛔ 게이트는 걸지 않았다 — 공식전 전체로 확대되면 G12에 추가한다.
