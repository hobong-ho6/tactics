...
- DB: players **200** · player_matches **4,175** · team_match_stats **65** · match_reports **29**
  ...
  **player_shirt_numbers 26**(신설) · understat_player_matches **5,634** · observations **374**.
...
- 회귀: **G1~G13** 전항 통과(⭐**G13 2026-09-01 신설**). 커널 회귀검증 **14/14 소수점 3자리 재현**(08-31, 빌라 영입 전수 수집 시).
...
### 2026-09-01 · ⭐⭐ 게이트 **G13 「조용한 이중화」 신설** (obs#374)

- **FK가 성립해서 G6가 원리적으로 못 잡는 부류만** 모았다. 4항:
  ⑴ **동일인 2-id**(`players`의 fotmob_id·sofascore_id 중복) ⑵ **이중 기록**(같은 `(player_id, match_id)`에 2행)
  ⑶ **match 링크 결손**(`match_id` NULL인데 `event_id`로 특정 가능) ⑷ **team_code ↔ 대회 성격 불일치**
- ⭐ ⑷는 **코드 목록을 하드코딩하지 않는다** — 「한 team_code가 클럽 대회와 대표팀 대회에 **동시에** 쓰이면 위반」이라는
  불변식을 쓴다. 새 코드가 늘어도 자기유지된다. ⚠️ **「FIFA Club World Cup」이 `%World Cup%`에 걸리는 함정을 제외**해야 한다.
- ⭐ ⑶은 ⑵의 **사각지대를 메우는 짝**이다 — 잭슨 08-05 1쌍이 정확히 그 틈에 숨어 있었다(한쪽 match_id가 NULL).
  그래서 지시는 「3종」이었지만 **4항으로 구현**했다.
- ✅ **회귀 검증**: 현재 DB는 4항 전부 0이고, **정리 전 백업 4종에서 전부 검출된다** —
  before-essugo-merge(1/77/20/3) · before-dedup77(0/77/20/3) · before-teamcode(0/0/20/3) · before-nt(0/0/0/3).
  네 검사가 **각각 독립적으로 다른 결함 상태**를 잡는다. `export.py`가 게이트를 자동 강제하므로 위반 시 익스포트가 막힌다.
- ⚠️ **한계 2가지**: ⑷는 어떤 코드가 **오직 잘못된 쪽에만** 쓰이면 통과한다 ·
  ⑵⑶은 `matches`가 같은 경기를 두 id로 갖는 경우를 못 본다(2026-08-16 Como 83/84는 **비공개 더블헤더 1·2차전**으로 진짜 별개 경기임을 확인했다).
- 문서: `docs/00-overview.md`의 게이트 목록을 **G1~G13**으로 갱신했다.
...
- **08-26**: CHE 풀럼전 **영상 축 완결**(obs#347) + **경기 전용 처방 재판정**(obs#348) —
  ...
  (시즌 정본은 경기 누적으로만). ⏰ 이 단계를 match-watch 표준 파이프라인에 넣을지는 **아직 미확정**.
- **08-27~30**: transfer-watch 다수회차(고레츠카·잭슨·왓킨스 CONFIRMED 승격 + squad_entries 반영, 신규 등재 5건) ·
  ⭐ **이벤트 축 수집기 신설**(`core.sofascore.js_event_collect` + `scripts/collect_event.py`, `f770dff`) ·
  AVL LM/RM 슬롯 x 재검토 **표본 부족 보류**(obs#349).
- **08-24~25(아카이브)**: 적합값 드리프트 20→8→4 · 좌우 이봉 7명(린델뢰프 .75→LCB .952) ·
  **전후(높이) 이봉 0건**(obs#342 — 사이드는 범주형, 높이는 연속형) · 게이트 3건 신설(G12 2종·G9) ·
  스키마 확장 2건 · ATM 4-4-2 슬롯 신설 · 경기 히트맵 렌더 버그 3원인 수정.
...
5. **P3 · 병렬 중복 실행 방지**(obs#354) — 08-31에 같은 4경기를 2회 수집했다. 가드 미수립.
   ...
   ✅ **재발 감지 게이트는 G13으로 신설했다**(obs#374) — 아래 「종결된 항목」 참조.
