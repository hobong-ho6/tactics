...
> 실측 48명·국면 그리드 48·PPDA/def_x 3경기 · 경기 프리셋 48행 · measured 처방 17행 · 영상 요약 7편+주장 11행 · manager_profiles 12축.
> ⭐⭐ **뒤집은 판정 3건**: obs#812 고메스 09-16도 **징계 결장**(→ 「바클리 = 결원」 2경기 연속) · obs#821 **「ATM 백3 = 유럽 원정 한정」 기각**(아노에타 라리가 원정 백3 — 4-4-2도 「기본형」 아님) ·
> obs#819 「맥 알리스터만 잔류」는 영어권 다수 오류 — EFE 「아라우호가 유일한 생존자」가 옳다(불변규칙 10 실증).
> ⭐ 새 축: obs#816 LIV **세트피스 수비 경고**(세트플레이 xG 1.18 = 상대 총 xG 53%, 매체 0건) · obs#820 ATM 「진짜 9번」 = 조나단 데이비드(박스터치 49 ↔ 알바레스 18, 더비 09-20 다음 표본) · obs#817 obs#644 첫 반례 방향(뇨니 잔류·6번 누출 0건 — 결함은 **짝 선택**의 함수?).
> 정정: ATM stage Round 5→6 · 조 고메스 sofascore_id. 부상 추가: 비조 등 · 알바레스(DB 결손). ⏰ D+2/D+3 3건 예약(09-18 21:00~22:00).
> ⚠️ 이월: Tanswell 0건(브라우저 미시도) · ATM Marca·AS·Relevo·MD 미도달 · 불변규칙 10 결손(AVL 스페인어·프랑스어, ATM 프랑스어·이탈리아어, LIV 바스크어).
> ---
> **⬇️ 2026-09-15 세션 ⑭~⑰ 압축** — 전문은 커밋·obs에 있다(규칙 8).
> ⑰ AVL 1-2 포레스트 3일 추적 종결(`0971716`, obs#787~792): ⭐⭐ **obs#787 바클리 LDM = 「선택」 아닌 「결원」**(고메스 PL 3경기 징계·유럽 미적용 → 브뤼헤 XI에서 딱 한 명 바뀐 것) ·
> obs#790 6′ 린델뢰프 골라인 미판정(xg 0.4366 개인 최대, 5패스가 놓침) · `manager_profiles` rotation 「유럽=별도 팀 외형 일부는 징계 적용 범위의 산물 — 강제 변경을 분리하라」.
> ⑮⑯ **G18 신설**(공식전 한정 재집계 ↔ map25) — `[표본판정` 표식이 친선 혼입 5행을 가림 · 채움 전용화 · G16 90행 재검증(APPLIED 6) · **`canon()` 오탐 4쌍**(obs#779~786). `refresh_duty_applied.py`가 회차마다 재판정 후보를 보고.
> ⑭ ⭐⭐⭐ ATM 표기 충돌 메커니즘 = **푸빌이 백4→백3에서 +16~17 이동**(요렌테는 상수, obs#775 · 할 일 30) · `reproduction_limits` #14 · LIV 6번 축 미결(할 일 29).
> ---
> **⬇️ 2026-09-15 세션 ①~⑬ 압축** — 전문은 커밋·obs에 있다(규칙 8).
...
00. ⭐⭐ **P1 · 진화·내 구단 축 후속(⑲)** — ⑴ **FC27 시장이 열리면 `collect_futgg_prices.py --games 27` 일일 실행**(스케줄 등록은 사용자 판단) → 「전술 구현」 탭 순위가 시세로 재정렬된다.
    ...
    ⑵-a ✅ 09-18 **역할 가중 점수**(migration 038 `game_role_key_attrs` 287행 · 37역할 × 3/2/1 가중, **판단값 MEDIUM** — EA 미공개): 진화 순위 = tier(처방 역할 Role++/+) → Σ(가중×속성Δ) → OVR → 비용 → 단계.
       `player_evolutions.upgrades`는 fut.gg upgrades가 전부 0으로 오는 경로가 있어 **결과 카드 − 기준 카드**로 직접 계산(path_json 각 단계에 34속성 `attrs` 포함). 재수집은 pulled 날짜별로 쌓인다(09-18 167경로 · 09-17 174경로는 attrs 없음).
       가중이 틀렸다고 보면 이 표만 UPDATE 허용(파생 순위만 바뀜 — 038 머리말). 선수 화면 「진화 제안」도 같은 규칙.
    ⑵-c ✅ 09-18 선수 화면: 외부 링크 **FC27↗(fut.gg 카드) 1차**·FC26 sofifa 보조 · **대표 카드 선택**(카드 여러 장이면 썸네일 스트립, 기본값 = 내 구단 보유 카드 → 기본 카드, 선택은 localStorage — 보기 설정이라 DB 아님).
