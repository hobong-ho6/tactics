...
- ⭐ **[2026-09-09] 이적창 종료 후 배지**: `squad_entries.lh='CONFIRMED'`는 출처 표시로 보존하되, 화면 배지는 **「신규」**로 그린다(`compare.html`).
  「합류확정」은 미합류 상태를 뜻하므로 스쿼드 동결 뒤에는 오독을 낳는다(사용자 지적). 값을 `OWNED`로 덮지 않는 이유는 「이번 창 영입」 출처가 지워지기 때문.
