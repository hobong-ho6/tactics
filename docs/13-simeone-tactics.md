...
### 시장 — ⚠️ 2026-08-21 정정
⚠️ **어제 「알바레스 이탈설(시메오네 언급)」이라고 쓴 것은 부정확했다 — 시메오네는 문을 닫았다.**

⭐⭐ **훌리안 알바레스 = ATM 최대 사안**(`transfer_outgoing` MEDIUM-HIGH):
- **선수가 공개 이탈 요구** — 2026 월드컵 경기 후 「había terminado」 + **바르셀로나행 희망** 표명
- **바르셀로나 정식 오퍼**(회장 **라포르타** 확인) · **레알 약 €150m 시도**
- ⛔ **CEO Gil Marín이 「ni 200 millones de euros」로 전면 거부** · **시메오네 「No ha lugar」**
- 관중이 말라가전에서 야유했고 시메오네는 그를 변호하지 않았다
- ⚠️ **R1 결장은 이 사안이 아니라 피지컬**(월드컵 후 복귀 지연·차등 훈련). R2는 배제하지 않았다

⭐ **쇠를로트**: **08-12 근육 경직(구단 확인) 약 4주** → R1 결장. 뉴캐슬 관심(ATM은 €40m+ 요구),
유벤투스·페네르바흐체도 거론되나 **잔류 전망**이고 1월 이탈 가능성으로 서술된다.
⭐ **Cristian Romero가 실제로 아틀레티코 소속임을 확인**했다(SofaScore 스쿼드 829932) — 어제 미확인 항목.
⭐ 아틀레티코는 우리 원장에 **잭슨(첼시) 경쟁 클럽**으로도 등장한다 — CHE 축과 겹친다.
...
| 항목 | 상태 |
|---|---|
| `sofascore_id` | ✅ **팀 2836 + 스쿼드 23/23 전원 확정**(2026-08-21, 403 해제 후 `/api/v1/team/2836/players`). ⚠️ 「Miguel Llorente」(FotMob)=「**Miguel Cubo**」(SofaScore 2238907) — 등번호 29·생년 2008로 확증. ⛔ **그 뒤 08-21 오후 SofaScore가 다시 전 엔드포인트 403**이 돼 `382c03f`로 추가한 8명은 **sofascore_id 없이 FotMob 경로로 등재**했다 |
| 선수 실측·커널 적합 | ✅ **R1 적재 완료** — `player_matches` 16 · `squad_entries` **31**(실측 16 + 자리표시 15) · `prescriptions` 16 · `player_duties` **31**. ⚠️ 표본 1경기 |
| ⚠️ 자리표시 **15명** | ⑴ R1 미출전 벤치 **7명**(무소·히메네스·푸빌·그리말도·카르도소·J.카스티요·M.요렌테, `736fbb9`) ⑵ ⭐ **R1 경기일 명단 밖 1군 8명**(알바레스·쇠를로트·쿠티 로메로·르마르·바르가스·알바로 모레노·푸리치·라하도, `382c03f`) — `squad_entries`는 경기일 명단이 아니라 **전체 스쿼드**인데 원천이 R1 명단이어서 통째로 빠져 있었다. `map25`가 NOT NULL이라 **전부 0 그리드** + `fit_*` **NULL**(AVL 네델코비치 선례). 슬롯은 **추정**이다 |
| ⭐ `sort_order` | ✅ **31행 전원 부여**(`a8b25eb`) — 실측 16행 = 10 + slot_type 내 fit_sim 내림차순 / 자리표시 15행 = 90 + 이름순. 사유: ATM만 sort_order가 전 행 NULL이라 히트맵 기본 인선이 **자리표시를 6/11 선택**했다(NULL이 SQLite ASC 최상단 → label 가나다순). 부여 후 **0/11**. ⚠️ **깊이 차트가 「실제 서열」이 아니라 「그릴 수 있는 그리드 유무」를 반영한다** — 그리말도(€40m LB 1옵션)가 다니 마르티네스 뒤다. **R2 이후 재도출 대상**(obs#269) |
