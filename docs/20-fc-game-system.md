...
- **자율 판단 지수(Autonomy IQ, v1 아카이브 전용 파생 휴리스틱·EA 공식 아님)** (2026-07-05): 아래는 과거 설계 기록이며 **현행 v2 구현·화면에는 이 지수가 없다**. 로밍/다재다능 같은
  ...
  72–79 보통/<72 낮음. 당시 비교 카드와 스쿼드 카드에 표시했다. 이 수치가 게임의 실제 AI 판단을
  측정한다는 근거는 없고, 특히 **반응력의 작용은 EA 1차 자료에서 확인되지 않았다**(docs/22 §2.1).
  카마라 딥피벗 82는 당시 휴리스틱의 출력값이지 현행 처방이나 EA 실측값이 아니다.
...
커널 코사인은 **x에 대한 계단 함수**다(`placedMap`이 x와 가장 가까운 EA 변형을 고른다). 슬롯 x가
변형 간 중간점을 넘으면 선택 커널과 값이 통째로 바뀐다
— 맥긴 RM `wm_insidefwd/Balanced`는 x=60~75에서 **0.574**, x=80~100에서 **0.802**다.
⚠️⚠️ **[2026-08-24 정정] 정본은 `db/tactics.db`의 `slots` 테이블이다.**
종전 문장은 「정본은 툴의 `SQUAD_SLOTS`(=`CMP_SLOTS`)」였으나 그 파일(`fc26-heatmap.html`)은
**`archive/v1/`로 옮겨진 동결·읽기 전용 아카이브**다(CLAUDE.md 불변규칙 1). v2에서 커널이 실제로 읽는 곳은
`slots`이고, `core.kernel.best_fit_slot()`이 `SELECT x, slot_type FROM slots WHERE regime_id=? AND pos=?`로 조회한다.
⇒ **아래 표는 사본이며, 값이 어긋나면 `slots`가 이긴다.**

**AVL(regime 1) — 4-2-3-1 Wide**

| GK | LB | LCB | RCB | RB | LDM | RDM | LM | CAM | RM | ST |
|---|---|---|---|---|---|---|---|---|---|---|
| 50 | 13 | 37 | 63 | 87 | **38** | 62 | 14 | 50 | **85** | 50 |

### ⭐⭐ 4팀 슬롯 x 전수표 (2026-08-24 DB 전수 대조)

⛔ **팀마다 x가 다르다. AVL 값을 다른 팀에 재사용하면 안 된다**(불변규칙 7).
아래는 **커널과 동일한 조회**(`regime_id`+`pos` → `fetchone()`)로 생성한 **실효값**이다.

| pos | AVL(1) | CHE(2) | LIV(3) | ATM(4) | slot_type |
|---|---|---|---|---|---|
| GK | 50 | 50 | 50 | 50 | GK |
| LB | 13 | 14 | 16 | 14 | FB |
| LCB | 37 | 25 | 36 | 35 | CB |
...
`—` = 그 팀의 슬롯 세트에 **존재하지 않는 포지션**이다(값 0이 아니라 부재). 해당 pos로 `best_fit_slot`을 부르면 `KeyError`가 난다.

> ⭐ **`formation`이 실효값을 정한다 — 그리고 커널은 그걸 보지 않는다.**
> `slots`는 팀마다 여러 포메이션을 담는다:
> **AVL** 4-2-3-1 Wide·4-4-2 / **CHE** 3-4-2-1·5-4-1·3-4-3 / **LIV** 4-2-3-1 Wide / **ATM** 4-1-4-1.
> 그런데 `best_fit_slot`은 `formation`을 조건에 넣지 않고 **`fetchone()`으로 첫 행**을 집는다.
> - **AVL**: 두 포메이션이 공유 pos의 x를 **의도적으로 동일하게** 맞춰 뒀다(4-4-2 행 주석에 명시) ⇒ 충돌 없음.
>   4-4-2 전용 `LST 40`·`RST 60`만 추가로 존재한다.
> - ⛔ **CHE**: **8개 pos(CCB·LCB·LCM·LM·RCB·RCM·RM·ST)에서 포메이션마다 x가 다르다.**
>   실효값은 삽입 순서상 첫 행인 **3-4-2-1**로 결정된다 — 위 CHE 열이 그 값이다.
>   ⚠️ 5-4-1·3-4-3 기준으로 계산하려면 `formation`을 명시해 **직접 조회**해야 하고, 커널 호출로는 안 된다.
>   (참고: 5-4-1·3-4-3은 `LCB 31 · CCB 50 · RCB 66 · LCM 40 · RCM 60 · LM 14 · RM 86 · ST 50`)
> - **LIV·ATM**: 포메이션이 하나씩이라 모호성이 없다.

> ⚠️⚠️ **[2026-08-24 전수 대조 결과 — 사본 표가 두 번째로 틀렸다]**
> `.claude/skills/transfer-watch/SKILL.md`의 3팀 사본 표를 DB와 대조한 결과
> **CHE 8칸·LIV 9칸이 틀렸고 AVL 열만 전부 맞았다.** LIV 열은 통째로 **AVL 값 사본**이었다.
> 특히 구 표의 `CHE LDM 50 · RDM 61`은 **CHE에 존재하지도 않는 슬롯**이었다(CHE는 `LCM 45 · RCM 55`).
> 2026-08-14에 같은 표에서 `RM 86·RB 86`이 CHE 값인데 3팀 공통처럼 적혀 있던 것이 발견돼 한 차례 정정됐으므로
> **이번이 두 번째**다 — 불변규칙 7이 경고하는 「팀 축 혼선」이 사본 표에 반복해서 들어간다.
> ⇒ **사본 표를 신뢰하지 말고 `slots`를 조회하라.** 사본이 필요하면 이 절을 참조하되, 갱신 시 DB에서 재생성할 것.

> ✅ **[2026-08-14 obs#197 — AVL LM/RM x 유지]** 25/26 실측은 명목 LAM **30.97**(n=46),
> RAM **76.06**(n=39)로 14/85보다 안쪽이지만, 이는 선수 움직임의 결과이지 게임 포메이션 앵커가 아니다.
> 14→30.9와 85→76.1을 FC26의 **85개 역할/포커스 전수**에 적용해도 최근접 변형 변경 0건,
> 9개 역할군·85개 경기별 그리드의 argmax 변경 0건, 최대 적합도 변화 **0.000000**이었다.
> 특히 좌측 평균 30.97은 모든 조합이 같은 변형을 고르는 공통 버킷 상단 **31.0**에 붙어 있어,
> 표본 평균을 슬롯 x로 쓰면 작은 표본 변화가 커널 버킷을 흔든다. 따라서 정본 14/85를 유지하고
> 내향 움직임은 실측 그리드와 역할 커널에서 표현한다. FC27 커널 추가 시 이 x-sweep을 다시 수행한다.
