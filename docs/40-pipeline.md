...
### ⭐ 마이그레이션 체크리스트 (2026-09-15 신설 — 할 일 19, obs#767)

새 컬럼·테이블을 만들 때 **아래 4개를 반드시 판단하고 결과를 마이그레이션 주석에 남긴다.**
계기: `migration 031`이 `player_duties.applied_status`·`applied_note`를 추가했는데
**`G14_PROTECTED`를 함께 갱신하지 않아 3주간 무보호**였고, 그 사이
`refresh_duty_applied.py`가 사람이 쓴 `applied_note` 9행을 자동 템플릿으로 덮었다.

1. **`G14_PROTECTED`에 넣을 것인가** (`scripts/gates.py`) —
   ⛔ **기계적으로 넣지 말 것.** G14 ⑴은 「**새 값이 옛 값으로 시작하지 않으면 재작성**」으로 판정하므로,
   **정상 작업이 전문을 다시 쓰는 필드는 넣으면 안 된다**(예: `applied_status`는 `PROSE`→`APPLIED`가 정상인데
   prefix 불일치로 매번 걸린다). ⇒ **덧붙임만 하는 서술 필드**(원장 텍스트)는 넣고,
   **상태·판정 필드**는 넣지 않는 대신 **⑵의 보호를 건다**.
2. **자동 재계산 스크립트가 그 필드를 쓰는가** — 쓴다면 **기존 값에 사람 손이 섞일 수 있는지** 묻는다.
   섞일 수 있으면 **채움 전용**(값이 있으면 건너뜀) 또는 **「상태가 바뀔 때만 다시 쓴다」**로 만들고
   `--force-*` 탈출구를 둔다. 선례: `refresh_duty_applied.py --force-note` ·
   `write_video_summary.py --force-claims` · `index_transcripts.py`(report_id 채움 전용).
3. **허용 값 집합(어휘)이 있는가** — 있으면 **게이트로 묶는다**(선례: G16 `applied_status` · G17 `axis`/`verdict`).
   ⛔ 어휘 정본이 없으면 세션마다 다른 말이 들어온다(`pos_class` 450행 사고, obs#527).
4. **파생·집계 경로가 그 컬럼을 읽는가** — 읽으면 **결손이 무엇을 막고 있었는지** 먼저 확인한다.
   선례: `pos_class` NULL이 친선 경기를 집계에서 **우연히** 빼주고 있었고, 백필하는 순간 obs#126이 깨졌다(obs#773).
