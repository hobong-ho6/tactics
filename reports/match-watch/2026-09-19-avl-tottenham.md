...
### D+1-9. 하프별 수치 보충 (2026-09-20 신규 수집)

D+0에 비어 있던 `match_period_stats`를 채웠다(FotMob 동일 스냅샷 · pulled=2026-09-20 scope=full).
...
| 빌라 xG | 0.82 | **0.35** |
| 빌라 슛 | 9 | 6 |
| 토트넘 점유 / xG | 57% / 0.64 | **71% / 0.92** |

⭐⭐ **「공을 덜 갖고 덜 만들면서 더 넣었다」** — 후반에 빌라는 점유·xG·슛이 전부 내려갔는데 **그 구간에 2골을 넣었다**(67′ 잭슨 · 79′ 부엔디아).
claim #137(`away_vs_top_game_plan = defend_deep_then_transition`, APPLIED)의 **정량 근거**이고 obs#868(후반 우측 편중 백5)·def_x 30.6과 같은 방향이다.
⚠️ **xG로 경기력을 읽으면 정반대 결론이 나온다**(후반 xG는 토트넘이 2.6배) — 이 축에서 xG는 전술 의도가 아니라 **상대 압박의 지표**다. obs#891.
...
## D+3 추적 (2026-09-21) — 3일 추적 마지막 회차

> ⚠️ 예정보다 하루 이르다(사용자 요청으로 D+2 직후 같은 날 실행 — 원래 예약은 09-22 21시). **D+3의 성격을 그대로 지켰다**:
> 새 서사를 더 모으는 대신 **같은 경기의 국면 그리드·스탯으로 판정을 닫았다**(코번트리전 선례와 동일 방식).
> WhoScored matchId `1983589` 재수집(scope=full · **1,627건** — D+2와 동일 건수, 같은 Opta 스냅샷 확인).

### D+3-1. ⭐⭐ HELD 2건 종결

#### claim #166 「롱볼이 토트넘 풀백을 묶어둔다」 — **APPLIED**

D+2가 건 재판정 조건(롱볼 시도 직후 60초 창으로 좁혀 상대 풀백 x를 대조)을 집행했다. AVL 롱볼 54회.

| | 창 밖 | 창 안(롱볼 후 60초) | Δ | Welch t |
|---|---|---|---|---|
| 로버트슨(LB) | x̄65.1(n=40) | x̄52.3(n=59) | **12.7** | 3.11 |
| 포로/그레이(RB) | x̄55.6(n=42) | x̄45.0(n=33) | 10.6 | 1.80 |
| 합산 | x̄60.2(n=82) | x̄49.7(n=92) | **10.5** | **2.98** |

로버트슨은 단독으로도 유의하고, 합산도 유의하다. ⇒ **APPLIED**.
⚠️ D+2의 기각 근거(로버트슨 전반→후반 이벤트 수 29→51 증가)는 **잘못된 지표였다** — 이벤트 수는 전진 빈도가 아니라 관여 빈도이고, 그 관여의 순간 위치가 눌려 있으면 억제는 여전히 성립한다. **재판정 조건을 걸 때 지표 자체가 가설을 재는 것인지 먼저 검산해야 한다.** obs#938.

#### claim #176 「3-0 이후 집중이 풀렸다」 — **REJECTED(확정)**

D+2가 건 재판정 조건(3-0 이후 구간 PPDA·def_x를 직전 구간과 대조, 유의하게 나빠지면 APPLIED)을 집행했다. 79′(부엔디아 3-0) 기준 분할.

| 구간 | PPDA | def_x |
|---|---|---|
| 0~79′ | 16.29(277/17) | 29.3(n=131) |
| **79~98′(3-0 이후)** | **6.40**(64/10) | **36.3**(n=35) |

PPDA는 낮아졌고(더 압박적) def_x는 높아졌다(라인이 더 전진) — **가설과 정반대 방향**이다. ⇒ **REJECTED 확정**.
대안 가설(claim #142 86′ 2차 처리 실패·claim #143 97′ 세트피스, 둘 다 이미 APPLIED)이 여전히 더 구체적이며, 이 두 실점을 「하나의 이완」으로 묶으면 안 된다는 D+2-10의 결론이 다시 확인됐다. obs#939.

### D+3-2. claim #167 「카마라·고메스 어느 쪽도 배급을 지배하지 않는다」 — 조건 미충족, **PENDING 유지**

재판정 조건(동시 선발 3경기에서 CB 듀오가 팀 패스 1·2위를 반복 차지)을 검증했다. 카마라·고메스 동시 선발 공식전은 이 경기까지 4경기(08-12 슈퍼컵·08-23 PL·09-08 UCL·09-19 PL)이나 **08-23은 `stats_json` 결손**으로 패스 집계가 안 된다.

| 경기 | 패스량 1·2위 | 카마라·고메스 순위 |
|---|---|---|
| 08-12 슈퍼컵 | 부엔디아·캐시 | 8위·9위 |
| 09-08 UCL | **파우 토레스·린델뢰프(CB 듀오)** | 3위·5위 |
| 09-19 이 경기 | 밍스(CB)·카마라 | 2위(카마라)·순위 밖(고메스) |

「CB 1·2위」 패턴은 3경기 중 **1경기(09-08)에서만** 나타난다 — 반복 조건 미충족. PENDING 유지, 조건 불변.

### D+3-3. claim #169 「맥긴이 침투를 놓친다」 — 조건 미충족, **HELD 유지**

독립 2소스 검색을 재시도했다(영어 「McGinn tracking runners Tottenham Aston Villa deep block」) — 신규 0건, 기존 하이라이트·경기 리포트 재인용뿐이었다. 같은 지적의 2경기째 누적도 없다(`video_impl_claims`에 `field='deep_block_runner_tracking'` 행이 이 1건뿐). 조건 그대로 유지.

### D+3-4. 마지막 소스 재시도 — 72′ 만잠비 교체 사유

D+1·D+2에 이어 **3회차 연속**으로 72′ 교체(체력 관리 vs 무릎 충격 관리)의 **직접 진술이 0건**임을 확인했다.

- 🇨🇭 `site:blick.ch Manzambi Ausgewechselt Tottenham` — Blick 사이트 자체 검색 경로(`/sport/fussball/international/search`)는 **404**(경로 자체가 없다 — D+2까지 애그리게이터 재인용으로 우회했던 이유가 확인됐다). Google 사이트 검색은 이적 관련 구 기사만 반환.
- 「Manzambi ausgewechselt Grund 19. September」(독어) — 신규 0건. 프리미어리그·Goal·Straits Times 등 결과 리포트만 반환.
- ⇒ **구조적 부재로 결론**(3회차 연속 0건 — 불변규칙 10 다국어 소진 후 종결 기준과 동일하게 적용). 교체 사유는 D+2-6의 「체력 관리 + 무릎 충격 관리 복합」 추정을 **최종 상태**로 남긴다.

### D+3-5. 3일 추적 종결 요약

| 항목 | 상태 |
|---|---|
| claim #137 원정 게임플랜 | APPLIED(3중 확인) |
| claim #139 카마라 백3 | REJECTED(확정) — 대체 claim #178(수비 완결성 목적) APPLIED |
| claim #141 GK 롱킥 루트 | APPLIED(확증) |
| claim #142 86′ 2차 처리 실패 | APPLIED(보강) |
| claim #143 97′ 세트피스 | APPLIED |
| claim #144 GK 11번째 필드 플레이어 | REJECTED |
| claim #166 롱볼 풀백 억제 | **APPLIED(D+3)** |
| claim #167 카마라·고메스 배급 비지배 | PENDING — 재판정 조건: 동시 선발 3경기 CB 1·2위 반복(현재 1/3) |
| claim #169 맥긴 러너 추적 실패 | HELD — 재판정 조건: 독립 2소스 또는 2경기 누적(현재 0) |
| claim #176 3-0 이후 이완 | **REJECTED(D+3, 확정)** |
| claim #178 카마라 하강의 목적=수비 완결성 | APPLIED(깊이·형태는 범위 밖) |

**PENDING/HELD 잔여 2건**(#167·#169) 모두 이번 경기의 표본만으로는 닫을 수 없는 **구조적 누적 조건**이다 — 다음에 카마라·고메스가 동시 선발하는 경기, 또는 맥긴 관련 동일 지적이 다시 나오는 경기에서 재판정한다.

**`match_reports.id=47` status: `draft` → `complete`.** G12 기준(필수 섹션 전체·출전 16명 전원 `match_player_reports`·경기 전용 `match_game_setups` 1행·선발 11명 `match_player_prescriptions`) 충족 확인.

**`manager_profiles` 갱신**: 이번 회차는 **갱신 불필요** — D+3의 결론(#166 APPLIED·#176 REJECTED)은 이미 D+0~D+2가 반영한 축(`buildup`의 롱볼 설계·`situational`의 후반 백5)을 **보강**할 뿐 새 axis를 열지 않는다.

**게이트**: G1~G20 전항 통과(아래 완료 절차 실행 후 확인).

## D+3 이후 추가 수집 (2026-09-22)

> D+3 종결(§위) 이후 하루가 더 지나며 나온 지연 게시분을 마지막으로 훑었다. **판정을 다시 열지 않는다** — 이미 닫힌
> claim에 대한 반증은 없었고, 새로 확인된 것은 전부 **기존 판정을 뒷받침하는 보강 자료**다.

### 1. ⭐ BBC 프리미어리그 5라운드 위클리 베스트일레븐 — 스즈키·만잠비 동시 선정

ゲキサカ(2026-09-21 22:27 발행)가 BBC 해설위원 **트로이 디니**의 선정 코멘트를 전했다.

> 「彼はトッテナム戦で2失点したが、(3点リードした)ビラにそれ以上、本気で攻める気はなく、失点について彼に責任はないだろう」
> 「彼はゴール前を完全に支配し、プレミアリーグにもすぐに順応した」
> (그는 토트넘전에서 2실점 했지만, [3골 리드한] 빌라가 그 이상 진심으로 공격할 마음이 없었으므로 실점에 대한 책임은 그에게
> 없을 것이다 / 그는 골문 앞을 완전히 지배했고 프리미어리그에도 곧바로 적응했다) — confidence: MEDIUM(3차 평가, 해설위원 개인 선정)

베스트일레븐 11명 중 **GK 스즈키·MF 만잠비 둘 다 아스톤 빌라**다 — 외부 매체가 이 경기의 두 핵심 기여자(스즈키 실측 평점 7.0·
만잠비 결승골)를 독립적으로 재확인한 것이다. obs#941.

⚠️ **스즈키 본인의 이 경기 코멘트는 D+0~D+3+이 회차까지 5회 연속 검색 0건이다.** 이번에 시도한 검색어(일본어):
「鈴木彩艶 代表 トッテナム戦 コメント」「鈴木彩艶 コメント「」トッテナム ロングフィード」「site:gekisaka.jp 鈴木彩艶 コメント」.
전부 매체·해설위원의 평가이지 본인 1차 발언이 아니다. **구조적 부재로 최종 종결** — 일본 대표팀 소집 시점(A매치 주간)
회견을 다음 기회로 남긴다.

### 2. 전반 20분 스즈키 세이브 타이밍 신규 확인

Read Aston Villa(James Chettle, 2026-09-20 21:20 발행)가 Sky Sports를 재인용:

> 「Sky Sports reported that Suzuki was forced into four saves inside the opening 20 minutes.」
> (스카이스포츠는 스즈키가 전반 20분 안에 4회 선방을 강요받았다고 보도했다.)

DB에는 총 세이브 수(5회)만 있고 시간 분해가 없었다 — **전반 초반 집중 압박**이라는 기존 서술(§2-1a 이라올라 발언
대조와 같은 패턴, 이번엔 상대인 토트넘 쪽)에 구체적 숫자를 더한다. 같은 기사는 밍스-린델뢰프 조합을
"파우 토레스(햄스트링, 포레스트전 부상) 결장에도 구조가 작동한다"는 에메리의 사전 발언에 대한 실증으로 서술한다 — 이미
§5·§6에 있는 밍스 LCB 실측과 정합한다. obs#942.
⚠️ 2차 인용(Read Aston Villa → Sky Sports)이라 원문 Sky Sports 기사 직접 대조는 미수행.

### 3. ⛔ 등재 제외 판정 — 신규 2건

- `wY_0dKnv0jY`(UTV, 2026-09-19 라이브 워치얼롱 2시간48분) — **감정 반응·실시간 채팅 포맷**, 사후 편집된 전술 분석이
  아니다(제목 「LIVE … WATCHALONG」·설명 확인). D+2가 UTV 쇼츠 4편을 같은 기준으로 제외한 것과 동일 사유.
- `7TJfTLcbBMA`(Villa On Tour, 2026-09-20) — 원정 팬 브이로그 채널의 반응 영상(제목 「ABSOLUTE SCENES」·
  VPN 스폰서십 삽입 확인), 전술 내용 없음. 채널 성격(원정 동행 브이로그) 자체가 분석 포맷이 아니다.

### 4. 종결 확인

새 자료 3건(§1·2) 모두 **기존 판정을 뒤집지 않는다** — REJECTED/APPLIED 상태 변경 없음. `manager_profiles` 갱신
불필요(기존 axis 보강조차 아니다 — 개인 평가·타이밍 세부일 뿐 전술 축이 아니다). 이 경기의 서사 수집은 **여기서 완전히
닫는다**. `match_reports#47`는 `complete` 유지.

## 최종 점검 (2026-09-22)

> 앞 회차가 유일하게 열어 둔 문 — **일본 대표팀 소집 시점의 스즈키 본인 발언** — 하나만 확인하고, 팬 채널의 지연
> 게시분을 짧게 훑었다. ⛔ **판정을 열지 않는다.** 이 절은 이 경기 수집의 마지막 기록이다.

### 1. 🇯🇵 대표팀 소집 — 본인 발언은 이번에도 0건, 대신 **몸 상태 정보**가 나왔다

일본 대표팀은 **2026-09-21 치바 합숙으로 W杯 이후 첫 활동을 시작**했고(태풍 25호로 첫날은 실내 조정), 스즈키는
유럽조 11명에 포함돼 합류했다. 09-24 우루과이전·09-28 베네수엘라전(기린 챌린지컵)이 예정돼 있다.

**신규 사실 — 소속 클럽에서 옆구리 부상, 대표팀 별도 메뉴 조정**(합숙 2일차, 09-22):

> 「鈴木彩は所属クラブでプレーしている時に**脇腹**を少し痛めたため、**別メニュー調整**。**離脱はしない**という。」
> (스즈키 사이온은 **소속 클럽에서 플레이하던 중 옆구리를 조금 다쳐** **별도 메뉴 조정**. **이탈은 하지 않는다**고 한다.)
> — 日刊スポーツ 2026-09-22 16:55 · 같은 내용을 スポニチ·サンケイスポーツ·サッカーキング가 동일 발행일에 보도.
> サッカーキング는 출처를 **JFA 설명**으로 명기: 「所属クラブで**脇腹を痛めた**ためと説明があった」
> (소속 클럽에서 옆구리를 다쳤기 때문이라는 설명이 있었다). — confidence: HIGH(JFA 1차 설명, 4매체 일치)

⚠️ **이 경기로 귀속할 수 없다.** 「所属クラブでプレーしている時に」는 시점을 특정하지 않는다 — 이 경기(09-19,
90분 풀타임) 외에 코번트리전(09-16)·클럽 훈련도 후보다. **직접 귀속 주장을 세우지 않는다.** 이 경기 §5의 스즈키
실측(5세이브·평점 7.0·2번째 골 기점 롱킥)에 사후적으로 부상 맥락을 소급 적용하지 않는다 —
경기 중 부상 징후를 보고한 소스는 D+0~D+3 통틀어 0건이었다.

⛔ **본인의 이 경기 1차 발언은 6회차 연속 0건**이다. 이번 회차 검색어(일본어):
「鈴木彩艶 日本代表 会見 コメント 2026年9月」 ·「鈴木彩艶 トッテナム戦 アストンビラ 本人 コメント」 ·
「鈴木彩艶 日本代表 合流 取材対応 9月21日 ヴィラ プレミア 手応え」 ·「鈴木彩艶 脇腹 別メニュー 日本代表 ウルグアイ戦」.
합류일(09-21) 취재 대응 기사(中日スポーツ·フットボールチャンネル·FOOTBALL ZONE·サカノワ)에도 **본인 코멘트는 없고**
사진 캡션(「鈴木彩艶の話に松木玖生らが耳を傾ける」 — 스즈키 사이온의 이야기에 마쓰키 쿠류 등이 귀를 기울이다)뿐이다.
⇒ **구조적 부재 최종 확정.** A매치 주간이라는 마지막 문까지 열어 봤고 닫혔다.

### 2. 팬 채널·기자 지연 게시분 — 신규 0건

UTV·The Villans·1874·Jacob Tanswell(X/Bluesky)의 **월간 리뷰·시즌 회고 형식 재언급을 찾았으나 신규 0건**이다.
검색어: 「Jacob Tanswell Aston Villa Tottenham review September 2026」 ·
「"Under the Villa" OR "The Villans" OR "1874" Aston Villa Tottenham 3-2 tactical review podcast」 ·
「"Aston Villa" Tottenham Emery tactics "3-2" analysis September 22 2026 monthly review」.
반환된 것은 전부 **D+0~D+3에 이미 흡수했거나 이미 등재 제외 판정한 것**이다 — Read Aston Villa 09-20 2편
(「86% tactical shift」는 §8-2·§6에 반영 · 「Late Spurs Scare / game management」 논지는 **D+3-1의 claim #176
REJECTED 확정으로 이미 실측 기각**), VAVEL·Yahoo Sports 회견 재인용, 토트넘 관점 팟캐스트(§8-2에서 데 제르비
회견 전문으로 이미 확보). **팬 채널의 신규 사후 편집 전술 콘텐츠는 없다.**

### 3. 종결 선언

신규 자료는 **§1의 옆구리 부상 1건뿐**이고, 그것은 이 경기에 귀속되지 않으며 **어떤 판정도 뒤집지 않는다**.
`video_impl_claims`·`match_reports.status`·`manager_profiles` **전부 무변경**(DB 쓰기 없음 — 리포트 기록만).
PENDING/HELD 잔여 2건(#167 카마라·고메스 배급 · #169 맥긴 러너 추적)은 **구조적 누적 조건**이라 이 경기 안에서는
영원히 닫히지 않는다 — 다음 해당 경기에서 재판정한다.

⛔⛔ **이 경기(matches#111 · match_reports#47)는 재점검하지 않는다.** D+0·D+1·D+2·D+3·D+3 이후·최종 점검까지
**6회차** 수집했고, 열려 있던 마지막 경로(대표팀 소집 회견)까지 닫혔다. 이후 이 경기에 대한 새 정보가 나온다면
그것은 **다른 경기의 리포트에서 누적 근거로 인용**할 일이지, 이 리포트를 다시 여는 일이 아니다.
